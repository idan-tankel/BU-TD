...
def main(language_list:list)->None:
  #  cmd_args = parser.parse_args()
    Raw_data_set = DataSet('/home/sverkip/data/Create_dataset_adapting_to_all_datasets/data') # Getting the raw data.
    parser = Get_parser()
  ...
  #  Omniglot_loader = EmnistLoader(parser.store_folder)
    ...
    parser.CHAR_SIZE = Raw_data_set.LETTER_SIZE
    ...
    for k, (ds_type, cur_nexamples) in enumerate(zip(ds_types, nexamples_vec)):
       ...
       #     adj_types=[0]
        ...
        for job_id in jobs_range:
            ...
            args = (parser, Raw_data_set, job_id, range_start, range_stop, examples[range_start:range_stop], storage_dir, ds_type, nclasses, job_chunk_size,   augment_sample,  parser.folder_split, parser.folder_size)
