...
# %% command line options # TODO - check what to do with this
def create_parser():
...
def load_raw_data(raw_data_fname):
    ...
    if not os.path.exists(raw_data_fname):
        print("no such file:", raw_data_fname,
              " ,this file is the basis for creating the dataset, can be downloaded from Liav's github")
        sys.exit(-1)
...
def main():
    ...
    available_test_details, available_test_persons, person_test_valid, person_train_valid = act_on_generalize(
        gen_hide_many,
        generalize,
        labels_raw,
        npersons,
...
def act_on_generalize(gen_hide_many, generalize, labels_raw, npersons, npersons_per_example):
    if generalize:
    ...
    return available_test_details, available_test_persons, person_test_valid, person_train_valid
