...
from common.common_functions import set_datasets_measurements, clear_fig
from persons.code.v26.Statistics.LaunchStatistics import LaunchStatistics
from v26.Configs.ConfigClass import ConfigClass
...
from v26.models.BU_TD_Models import *
...
def check_statistics(train_opts, datasets, config, inputs_to_struct, logger):
    statistics = LaunchStatistics(datasets=datasets, config=config, opts=train_opts, inputs_to_struct=inputs_to_struct,
                                  logger=logger)
...
def main():
    ...
    train_opts = init_train_options(model, args, num_gpus, scale_batch_size, ubs, batch_size, nbatches_train,
                                    ...
                                    config.RunningSpecs.num_epochs)

    if config.RunningSpecs.isFit:
        fit(train_opts, the_datasets, config)

    if config.RunningSpecs.getStatistics:
        check_statistics(train_opts, the_datasets, config, inputs_to_struct, logger)
    ...
    for sample_index in range(len(samples.image)):
        go_over_sample(config.Strings.features_strings, fig, flag_to_comp, sample_index, model_opts, n, nfeatures, outs,
                       preds,
                       samples)
    # %% percent correct
...
def go_over_sample(features_strings, fig, flag_to_comp, sample_index, model_opts, n, nfeatures, outs, preds, samples):
    clear_fig(fig)
    # we only have existence information about all the avatars, without order
    present = str(samples.label_existence[sample_index].nonzero()[0].tolist())
    tit = 'Present: %s\n' % present
    fl = samples.flag[sample_index]
    avatar_id, feature_id = flag_to_comp(fl)
    feature_value, predicted_feature_value = predict(avatar_id, feature_id, sample_index, nfeatures, outs, samples,
                                                     model_opts)
    ...
    ax.axis('off')
    plt.imshow(samples.image[sample_index].astype(np.uint8))
    ax = plt.subplot(1, n, 2)
    ax.axis('off')
    plt.imshow(samples.image[sample_index].astype(np.uint8))
    curseg = samples.seg[sample_index].squeeze()
    ...
    predicted_existing_avatar_ids = preds[sample_index]
    font = get_font(feature_value, predicted_feature_value)
    title_with_td_loss(font, gt_str, sample_index, model_opts, n, outs, pred_str)
    print(sample_index)
...
def predict(avatar_id, feature_id, sample_index, nfeatures, outs, samples, model_opts):
    if model_opts.flag_at is FlagAt.NOFLAG:
        pred = outs.task[sample_index].argmax(axis=0)
        pred = np.array(pred).reshape((-1, nfeatures))
        predicted_feature_value = pred[avatar_id][feature_id]
        ...
        if isinstance(outs.task, list):
            predicted_feature_value = outs.task[sample_index][0].argmax()
            # predicted_feature_value = get_predicted_feature_value(outs.task, samples.flag, sample_index)
        else:
            predicted_feature_value = outs.task[sample_index].argmax()
...
def model_loss_and_accuracy(model_opts):
    if model_opts.flag_at is FlagAt.NOFLAG:
        if not model_opts.head_of_all_features:  # TODO - change it from config
            ...
            model_opts.task_accuracy = multi_label_accuracy_weighted_loss_only_one
    else:
        if not model_opts.head_of_all_features:
...
def init_model_opts_2(args, base_tf_records_dir, config: ConfigClass, flag_at, flag_size,
                      inputs_to_struct,
