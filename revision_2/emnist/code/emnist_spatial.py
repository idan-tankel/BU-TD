...
import os
import emnist.code.v26.cfg as cfg
from common.common_classes import MeasurementsBase
from emnist.code.v26.Statistics import write_model_output
from common.common_functions import *
from emnist.code.v26.funcs import *
import argparse

from transformers.objectss_of_tasks.RightOfObjectTask import DatasetInfo
...
def main():
    ...
    if dummyds:
        if flag_at is FlagAt.NOFLAG:
            from v26.emnist_dataset import EMNISTAdjDatasetBase, calc_label_adj_all, \
                inputs_to_struct_label_adj_all as inputs_to_struct
        ...
        if flag_at is FlagAt.NOFLAG:
            from v26.emnist_dataset import EMNISTAdjDatasetLabelAdjAllNew as dataset, \
                inputs_to_struct_label_adj_all as inputs_to_struct
        else:
        ...
        if normalize_image:
            ...
            train_dl = DataLoader(train_ds,
                                  ...
                                  shuffle=False,
                                  pin_memory=True)
            mean_image, _ = get_mean_image(train_dl, inshape, inputs_to_struct)
            train_ds = dataset(os.path.join(base_samples_dir, 'train'),
                               nclasses_existence,
                               ndirections,
                               nexamples=nsamples_train,
    ...
    # Statistics
    # write the output of test(which is Combinatorial Generalization) - into file
    write_model_output(train_opts, test_dataset, logger, model, inputs_to_struct, nclasses)
