...
class EMNISTAdjDatasetBase(data.Dataset):
    def __init__(self, root, nclasses_existence, ndirections, std, mean, nexamples=None, split=False, mean_image=None):
        self.root = root
...
class EMNISTAdjDatasetNew(EMNISTAdjDatasetBase):
    def __getitem__(self, index):
        ...
        label_existence = label_existence.float()
        adj_type, char = flag
        adj_type_ohe = torch.nn.functional.one_hot(torch.tensor(adj_type).long(), self.ndirections)
        char_ohe = torch.nn.functional.one_hot(torch.tensor(char).long(), self.nclasses_existence)
        flag = torch.cat((adj_type_ohe, char_ohe), dim=0)
        flag = flag.float()
        label_task = label_task.view((-1))
        return img, seg, label_existence, label_all, label_task, id, flag
