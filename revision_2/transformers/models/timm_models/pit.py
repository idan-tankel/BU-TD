...
class Transformer(nn.Module):
    def __init__(
            ...
            num_instructions: int = 0, ratio_this_and_next: int = 0):
        super(Transformer, self).__init__()
        self.layers = nn.ModuleList([])
        embed_dim = base_dim * heads
        embed_dim = embed_dim + (max(heads, num_instructions) if num_instructions > 0 else 0)
...
class ConvHeadPooling(nn.Module):
    def __init__(self, in_feature, out_feature, stride, padding_mode='zeros'):
        ...
        # TODO - how to handle the num_instructions here? (add to in_feature? out_feature?)
        # out_feature = out_feature + num_instructions * previous_ratio
        # in_feature = in_feature + num_instructions
        self.conv = nn.Conv2d(
...
class PoolingVisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            distilled=False, attn_drop_rate=.0, drop_rate=.0, drop_path_rate=.0, num_instructions=0):
        ...
        added_size_for_the_instructions = max(heads[0], num_instructions) if num_instructions > 0 else 0
        self.cls_token = nn.Parameter(
            torch.randn(1, self.num_tokens, base_dims[0] * heads[0] + added_size_for_the_instructions))
        self.pos_drop = nn.Dropout(p=drop_rate)

        transformers = []
        # stochastic depth decay rule
        dpr = [x.tolist() for x in torch.linspace(0, drop_path_rate, sum(depth)).split(depth)]
        ratio_this_and_next = 0
        for stage in range(len(depth)):
            pool = None
            if stage < len(heads) - 1:
                # previous_ratio = ratio_this_and_next
                # ratio_this_and_next = (base_dims[stage + 1] * heads[stage + 1]) // (base_dims[stage] * heads[stage])
                pool = ConvHeadPooling(int(base_dims[stage] * heads[stage] + heads[stage] * num_instructions / heads[0]),
                                       int(base_dims[stage + 1] * heads[stage + 1] + heads[stage + 1] * num_instructions /
                                       heads[0]),
                                       stride=2)
            transformers += [Transformer(
                ...
                num_instructions=num_instructions, ratio_this_and_next=ratio_this_and_next)
            ]
        self.transformers = SequentialTuple(*transformers)
        self.num_features = self.embed_dim = base_dims[-1] * heads[-1] + int(heads[-1] * num_instructions / heads[0])
