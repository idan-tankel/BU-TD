...
def GroupNorm():
    f=lambda num_channels, dims=2: nn.GroupNorm(GROUP_NORM_LOOKUP[num_channels],num_channels)
    return f

def BatchNormNoStatsNoAffine(num_channels, dims=2):
    if dims==2:
        norm = nn.BatchNorm2d(num_channels,track_running_stats=False,affine=False)
    else:
        norm = nn.BatchNorm1d(num_channels,track_running_stats=False,affine=False)
    return norm
...
def LocalResponseNormWBN(size): 
    def fun(num_channels, dims=2): 
        if dims==2:
            return nn.LocalResponseNorm(size) 
        else:
            return nn.BatchNorm1d(num_channels,track_running_stats=False,affine=False)
    return fun

def LocalResponseNorm(size): 
    def fun(num_channels, dims=2): 
        if dims==2:
            return nn.LocalResponseNorm(size) 
        else:
...
def init_module_weights(modules):
    if False:
        # use PyTorch's defaults which are kaiming uniform (with leaky-relu) for Conv2D and Liner 
        for m in modules:
            if isinstance(m, Hadamard):
                nn.init.kaiming_uniform_(m.weights,nonlinearity='relu')
    elif False:
        for m in modules:
...
class BasicBlockLatShared(nn.Module):
    ...
    def __init__(self, shared, norm_layer, activation_fun,use_lateral):
...
class ResNetLatShared(nn.Module):
    ...
    def __init__(self, opts, shared, lateral_active):
        super(ResNetLatShared, self).__init__()
        self.norm_layer = opts.norm_fun
        self.activation_fun = opts.activation_fun
        self.inshapes = shared.inshapes
        self.use_lateral = shared.use_lateral and lateral_active #incoming lateral
        filters = opts.nfilters[0]
        self.use_bu1_flag = opts.use_bu1_flag
        if self.use_bu1_flag:
    ...
    def _make_layer(self, blocks):
        ...
        for shared_block in blocks:
            layers.append(BasicBlockLatShared(shared_block, norm_layer, self.activation_fun,self.use_lateral))
...
class BUModel(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.trunk=ResNetLatShared(opts,bu_shared,lateral_active=False)
...
class BUTDModelShared(BUTDModel):
    ...
    def __init__(self, opts):
        ...
        bu_shared = ResNetLatSharedBase(opts)
        self.bumodel1=ResNetLatShared(opts,bu_shared,lateral_active=False)
        opts.use_top_flag = opts.use_bu2_flag
        self.bumodel2=ResNetLatShared(opts,bu_shared,lateral_active=True)
...
def save_model_and_md(model_fname,metadata,epoch, opts):
    ...
    if opts.scheduler:
        save_info['scheduler_state_dict'] = opts.scheduler.state_dict()

    torch.save(save_info, tmp_model_fname)
...
class DatasetInfo():
    ...
    def __init__(self,istrain,ds,nbatches,name,checkpoints_per_epoch=1,sampler=None):
        self.dataset = ds
    ...
    def do_epoch(self,epoch,opts):
        ...
        for inputs in self.dataset_iter:
            ...
            # if self.name=='val':
            #     if cur_batches % 500 == 0:
            #     
            #         # Here we save a checkpoint. It is automatically registered with
            #         # Ray Tune and will potentially be passed as the `checkpoint_dir`
            #         # parameter in future iterations.
            #         with tune.checkpoint_dir(step=cur_batches) as checkpoint_dir:
            #             path = os.path.join(checkpoint_dir, "checkpoint")
            #             torch.save(
            #                     (opts.model.state_dict(), opts.optimizer.state_dict()), path)
            #
            #         measurements_history = self.measurements.get_history()
            #         tune.report(loss=measurements_history[0], accuracy=measurements_history[-1],training_iteration=cur_batches)
...
def train_step(inputs,opts):
    ...
    loss.backward()
    opts.optimizer.step()
    if type(opts.scheduler) in [optim.lr_scheduler.CyclicLR,optim.lr_scheduler.OneCycleLR]:
         opts.scheduler.step()
         # logger.info('next learning rate: {}, {}'.format(opts.optimizer.param_groups[0]['lr'],opts.scheduler.get_last_lr()))
...
def fit(opts,the_datasets):
    ...
    if opts.checkpoint_dir:
        model_latest_fname = os.path.join(opts.checkpoint_dir, "checkpoint")
    else:
    ...
    last_epoch = -1
    model_found = False
    if opts.load_model_if_exists or opts.checkpoint_dir:
    ...
    for epoch in range(st_epoch, end_epoch):
        ...
        if opts.first_node:
            ...
            if opts.save_model or opts.optimize_hyperparams:
                ...
                if opts.optimize_hyperparams:
                    from ray import tune
                    # Here we save a checkpoint. It is automatically registered with
                    # Ray Tune and will potentially be passed as the `checkpoint_dir`
                    # parameter in future iterations.
                    with tune.checkpoint_dir(step=0) as checkpoint_dir:
                        model_latest_fname = os.path.join(checkpoint_dir, "checkpoint")                
                        save_model_and_md(model_latest_fname,metadata,epoch,opts)
                    measurements_history = save_by_dataset.measurements.get_history()
                    tune.report(loss=measurements_history[0], accuracy=measurements_history[-1],training_iteration=epoch)
