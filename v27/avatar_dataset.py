...
from v27.funcs import AutoSimpleNamespace
...
class AvatarDetailsDatasetRawComposition(AvatarDetailsDatasetRaw):
    def __getitem__(self, index):
        is_train = self.root.endswith('train')
        root = self.get_root_by_index(index)
        fname=os.path.join(root,'%d_img.jpg' % index)
        img = Image.open(fname).convert('RGB')
        fname=os.path.join(root,'%d_seg.jpg' % index)
        ...
        if is_train:
            ...
            first_feature =  flag[1]
            remaining_features = np.setdiff1d(range(self.nfeatures), first_feature)
            av_feats = np.random.choice(remaining_features,1,replace=False)
            av_feats_ohe=torch.zeros(self.nfeatures)
            av_feats_ohe[first_feature]=1
            av_feats_ohe[av_feats]=1
        flag = torch.cat((av_ids_ohe,av_feats_ohe),dim=0)
        flag = flag.float()
        label_task = label_task.view((-1))
        loss_weight = av_feats_ohe.float()
        return img,label_existence, flag,seg,label_task,id,label_all, loss_weight
