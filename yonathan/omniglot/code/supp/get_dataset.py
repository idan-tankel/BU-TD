...
def get_dataset_for_spatial_realtions(args,data_fname, embedding_idx: int,direction:int) -> list:
    ...
    if args.ds_type is DsType.Omniglot:
     from supp.datasets import OmniglotDatasetLabelSingleTaskRight as dataset
    else:
    ...
    val_ds = dataset(os.path.join(data_fname, 'test'), args.nclasses, args.ntasks, embedding_idx, direction, args.nargs, nexamples=nsamples_test, split=True, mean_image=mean_image)
    ...
    val_dl = DataLoader(val_ds, batch_size=batch_size, num_workers=args.workers, shuffle=False, pin_memory=True,
                         sampler=test_sampler)
    ...
    the_datasets = [the_train_dataset, the_test_dataset,the_val_dataset]
