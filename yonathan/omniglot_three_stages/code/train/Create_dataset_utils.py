...
def Get_label_task(example:ExampleClass, infos:list,label_ordered:np.array,nclasses:int,keypoints:list)->tuple:
    ...
    """
    query_part_id = example.query_part_id_for_stages_1_2 # The index we create about.
...
def create_examples_per_sample(examples,sample_chars,chars, prng, adj_types,ncharacters_per_image,single_feat_to_generate,is_test,is_val,ngenerate):
    ...
    for adj_type in adj_types:
        valid_queries = range(1, ncharacters_per_image-1)
        if single_feat_to_generate or is_test or is_val:
            query_part_ids = [prng.choice(valid_queries)]
            query_part_ids_for_stage_3 = [prng.choice(range(ncharacters_per_image))]
        else:
            ...
            if cur_ngenerate == -1:
                ...
                query_part_ids_for_stage_3 = prng.choice(range(ncharacters_per_image), cur_ngenerate, replace=False)
        for idx, query_part_id in enumerate(query_part_ids):
            query_part_id_for_stages_1_2 = query_part_id
            query_part_id_for_stage_3 = query_part_ids_for_stage_3[idx]
            example = ExampleClass(sample_chars,query_part_id_for_stages_1_2,adj_type,chars,query_part_id_for_stage_3 )
...
def Get_data_dir(parser, language_list):
    ...
    base_storage_dir += 'test_offsets_110K' + str(language_list)
