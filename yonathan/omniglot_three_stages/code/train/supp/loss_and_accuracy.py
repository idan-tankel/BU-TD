...
class CYCLICUnifiedLossFun:
    ...
    def __call__(self, inputs: list[torch], outs_list: list,test_all_stages = False) -> float:
        """
        Args:
...
def accuracy(opts: nn.Module, test_data_loader: DataLoader,stages:list,embedding_idx:list=None) -> float:
    """
    Args:
    ...
    if stages == [0,1,2]:
        for inputs in test_data_loader:  # Running over all inputs
            ...
            opts.model.train() #
            outs = opts.model(inputs,is_train = False)  # Compute the output.
            outs = get_model_outs(opts, outs)  # From output to struct
            samples = cyclic_inputs_to_strcut(inputs, 2, outs[1])  # Make it struct.
            (_, task_accuracy_batch) = multi_label_accuracy_base(outs[2], samples, stage = 2)  # Compute the accuracy on the batch
        ...
        for inputs in test_data_loader:  # Running over all inputs
         ...
         #   load_running_stats(opts.model, 1)
            opts.model.train()  #
            outs = opts.model(inputs, is_train = False)  # Compute the output.
            outs = get_model_outs(opts, outs)  # From output to struct
            outs = outs[stage]
            ...
            num_correct_pred += task_accuracy_batch.sum()  # Sum all accuracies on the batches.
            num_zeros+= (samples.label_task == 194).sum()
        return num_correct_pred / num_samples  # Compute the mean.