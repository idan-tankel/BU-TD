...
class TDModel(nn.Module):
    def __init__(self, opts: argparse):
        """
        Args:
            opts: The model option to create the model according to.
        """
    ...
    def _make_layer(self, block: nn.Module, planes: int, num_blocks: int, stride: int = 1, inshapes: list = None,
                    ntasks: int = 1,index:int=0)->nn.Module:
        """
        Args:
            block: Basic TD stream block.
            planes: The out channels.
            num_blocks: Number of blocks in the layer.
            stride: The stride.
            inshapes:  Input shape.
            ntasks: Number of tasks the model should handle.
    ...
    def forward(self, inputs: list[torch]) -> list[torch]:
        """
        Args:
            inputs: bu_out - the output from the BU1 stream, flag the task+arg flag , laterals_in the laterals from the BU1 stream.

        Returns: The output of the TD stream.
        """
...
class BUStreamBUStream(nn.Module):
    ...
    def _make_layer(self, blocks: nn.Module) -> nn.ModuleList:
        """
        Args:
        ...
        for shared_block in blocks:
            ...
            layer = self.block(shared_block, norm_layer, self.activation_fun, self.inshapes, self.ntasks, self.model_flag,
                               self.is_bu2, self.orig_relus)
    ...
    def forward(self, inputs: list[torch]) -> tuple:
        """
        Args:
            inputs: x - the output from the previous layer, flag the task+arg flag , laterals_in the laterals from the previous stream if exists.

        Returns: The output of the BU stream.
...
class BUStreamShared(nn.Module):
    ...
    def _make_layer(self, planes: int, nblocks: int, stride: int = 1,num_blocks:int=0) -> nn.Module:
        ...
        Args:
            ...
            stride: The stride.
            num_blocks: Num blocks created so far in the stream.

        Returns: A bu_shared layer between BU1, BU2.

        """
        layers = []
        layers.append(self.block(self.inplanes, planes, stride, self.use_lateral,num_blocks))  # Add an initial block
        self.inplanes = planes * self.block.expansion
        for _ in range(0, nblocks - 1):  # Add nblocks - 1 preserving blocks.
...
class BUTDModel(nn.Module):
    def forward(self, samples: list[torch], stage:int ) -> list[torch]:
        ...
        Args:
            samples: The samples.
            stage: The stage we are currently training.

        Returns: The output from all streams in order to compute the appropriate loss.
        """
...
class BUTDModelShared(BUTDModel):
    def __init__(self, opts: argparse) -> None:
        """
        Args:
            opts: The model options to create the model according to.
        """
        ...
        if self.model_flag is FlagAt.SF:  # Storing the Task embedding.
            for i in range(self.ntasks):
               # self.task_embedding[i].extend(list(self.Head.taskhead[i].parameters()))
                ...
                for j in range(3):
                 self.Head_learning[i].append(list(self.Head.taskhead[i][j].parameters()))
        else:
            for i in range(self.ntasks):
                self.task_embedding[i].extend(list(self.Head.taskhead[i].parameters()))
...
def forward_stage(self, inputs,stage, out_prev, freeze,is_train):
    model = self.model
    if is_train: # We are in train mode.
     if freeze and stage == 1:
      freeze_model(model, False)
     if freeze and stage != 1:
      freeze_model(model, True)
    else: # We are in test mode.
     embedding_idx = self.embedding_idxs[stage]
     load_running_stats(model, embedding_idx)
     #  print("Stage = "+str(stage) +" Loaded running stats idx = " + str( embedding_idx))
    samples_stage = cyclic_inputs_to_strcut(inputs, stage, out_prev)
    out_stage = self.model(samples_stage, stage = stage)
   # out_stage = outs_to_struct_base(model)(out_stage)
    return out_stage
...
class CYCLICBUTDMODELSHARED(nn.Module):
    def __init__(self,opts):
        ...
        self.embedding_idxs = opts.embedding_idxs
 # TODO - CHANGE T0 PARTIAL FUNCTIONS.

    def forward(self, inputs: list[torch],is_train:bool,freeze:bool=False) -> list[torch]:
        """
        Args:
            inputs: The inputs to the model.
            is_train: Whether we training/testing the model.

        Returns: The output from all desired stages.
        """
        stages = self.stages
        outs = {}
        # Training stage 0.
        model = self.model
        test_all_stages = not is_train and len(stages) == 3
        if 0 in stages:
         out_stage_1 = forward_stage(self,inputs, 0, None, freeze, is_train)
         outs[0] = out_stage_1
        # Training stage 1.
        if 1 in stages:
         if test_all_stages :
          out_prev = outs_to_struct_base(model)(out_stage_1)
         else:
          out_prev = None
         out_stage_2 = forward_stage(self,inputs, 1, out_prev, freeze, is_train)
         outs[1] = out_stage_2
        # Training stage 2.
        if 2 in stages:
         if test_all_stages:
          out_prev = outs_to_struct_base(model)(out_stage_2)
         else:
          out_prev = None
         out_stage_3 = forward_stage(self,inputs, 2, out_prev, freeze, is_train )
         outs[2] = out_stage_3
        return outs
