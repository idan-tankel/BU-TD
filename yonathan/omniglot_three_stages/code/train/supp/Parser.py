...
def GetParser(samples_path, embedding_idxs,stages, checkpoints_per_epoch=1):
    """
    Args:
        samples_path: The path to the samples.
        embedding_idx: The embedding index.
        checkpoints_per_epoch: checkpoints per epoch.
        stages: The stages we are training.

    Returns: The model options.

    """
    path_fname = os.path.join(os.path.join('/home/sverkip/data/BU-TD/omniglot/data/samples',samples_path), 'metadata')
    ...
    language_dict = metadata.nclasses_per_language
    dict = {}
    initial_tasks = [18, 42, 5, 27]
    dict[0] = sum(language_dict[i] for i in initial_tasks )
    for i in range(1,51):
        dict[i] = language_dict[i-1]
    now = datetime.now()
    dt_string = now.strftime("%d.%m.%Y%H:%M:%S")
    model_path = 'DS=' + str(samples_path) + "_embedding_idx=" + str(embedding_idxs) + 'Stage=' + str(stages) + "Time=" + str(dt_string)
    #####################
    ...
    parser.add_argument('--grit_size', default = 4, type=list, help= "'")
    ...
    parser.add_argument('--initial_tasks', default=[49], type=list,  help='The initial tasks to train first')
    ...
    parser.add_argument('--ntasks', default = 1, type=int, help='Number of tasks the model should handle')
    ...
    parser.add_argument('--inshape', default=(3, 112, 300), type=tuple, help='The input image shape')
    ...
    parser.add_argument('--results_dir', default = '/home/sverkip/data/BU-TD/omniglot/code/data/results', type=str, help='The direction the model will be stored')
    parser.add_argument('--data_dir', default = '/home/sverkip/data/BU-TD/omniglot/data/samples/', type=str,  help='The direction the model will be stored')
    parser.add_argument('--nclasses', default = dict, type=list,  help='The sizes of the Linear layers')
    ...
    parser.add_argument('--embedding_idxs',default = embedding_idxs, type =list)
