...
class OmniglotDatasetLabelSingleTaskRight(OmniglotDataSetBase):
    ...
    def __init__(self, root: str, parser:argparse, embedding_idx: int,  nexamples: int = None, split: bool = False, mean_image: torch = None):
        ...
        """
        super(OmniglotDatasetLabelSingleTaskRight, self).__init__(root, parser.nclasses[embedding_idx[0]],parser.ntasks, nexamples, split,  mean_image)
        self.embedding_idxs = embedding_idx
        self.parser = parser
        self.ntasks = parser.ntasks
        self.direction = 0
        self.obj_per_row = 5
        self.grit_size = parser.grit_size
        self.num_grits = int(np.ceil(parser.inshape[2] / self.grit_size))
    ...
    def __getitem__(self, index):
        ...
        general_flags = []
        for i in range(3):
            task_emb_id = torch.tensor(self.embedding_idxs[i])
            # Creating the flag including task and argument one hot embedding.
            ...
            general_flags.append(general_flag)

        general_flag_1 = general_flags[0]
        general_flag_2 = general_flags[1]
        general_flag_3 = general_flags[2]
        ...
        label_task_stage_1 = (torch.div(label_task_stage_1, self.grit_size)).long() # The location of the character on the grit.
        '''
        ...
        label_task_stage_1 = torch.div(label_task_stage_1, self.grit_size, rounding_mode='trunc')
        ...
        #
        idx = sample.query_part_id_for_stage_3
        sample.keypoints[idx] = torch.tensor(sample.keypoints[idx])
        (x_location_neightboor, y_location_neighboor) = torch.div(sample.keypoints[idx], self.grit_size).long()
        flag_stage_3_x = torch.nn.functional.one_hot(x_location_neightboor,   self.num_grits)  # Making one hot embedding of the x coordinate.
        flag_stage_3_y = torch.nn.functional.one_hot(y_location_neighboor, self.num_grits)  # Making one hot embedding of the y coordinate.
        flag_stage_3 = torch.concat([flag_stage_3_x, flag_stage_3_y], dim=0).float()  # Making a one flag.
        label_task_stage_3 = label_all[0,idx]
        #
        return  img, label_all, label_existence,neighboor_character,general_flag_1,general_flag_2,general_flag_3, flag_stage_1, flag_stage_2, flag_stage_3 ,label_task_stage_1, label_task_stage_2, label_task_stage_3
