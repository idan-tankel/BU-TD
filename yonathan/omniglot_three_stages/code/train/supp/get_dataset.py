...
def get_dataset(opts: argparse,embedding_idx: int,  data_fname: str) -> list:
    """
    Args:
        opts: The model options we create the data loaders according to.
        embedding_idx: The embedding index.
        data_fname: The data source.
    ...
    train_ds = dataset(os.path.join(data_fname, 'train'), opts, embedding_idx,  nexamples = nsamples_train, split=True)
    # If normalize_image is True the mean of the dataset is subtracted from every image.
    batch_size = opts.bs
    if opts.normalize_image:
        train_dl = DataLoader(train_ds, batch_size = opts.bs, num_workers = opts.workers, shuffle=True, pin_memory=True)
    ...
    train_ds = dataset(os.path.join(data_fname, 'train'), opts, embedding_idx, nexamples = nsamples_train, split=True,mean_image = mean_image)
    test_ds = dataset(os.path.join(data_fname, 'test'), opts, embedding_idx, nexamples=nsamples_test, split=True, mean_image = mean_image)
