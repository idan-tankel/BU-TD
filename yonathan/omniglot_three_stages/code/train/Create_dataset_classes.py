...
class Sample:
    ...
    def __init__(self, infos, image, label_existence, label_ordered,sample_id, query_part_id,flag, label_task, keypoints,query_part_for_stage_3):
        """
        #Class containing all information about the sample, including the image, the flags, the label task.
        Args:
        ...
        self.query_part_id_for_stage_3 = query_part_for_stage_3
        self.label_task = label_task
        self.flag = flag
        self.keypoints = keypoints

class ExampleClass:
    def __init__(self,sample:Sample,query_part_id_for_stages_1_2:int,adj_type:int,chars:list,query_part_id_for_stage_3 ):
        """
        Args:
            sample: The sample.
            query_part_id: The index we query about.
        ...
        """
        self.sample = sample
        self.query_part_id_for_stages_1_2 = query_part_id_for_stages_1_2
        self.adj_type = adj_type
        self.chars = chars
        self.query_part_id_for_stage_3 = query_part_id_for_stage_3
...
class CharacterTransforms:
   # def __init__(self, prng: random, letter_size: int, label_ids: list, samplei: int, sample_chars: int, total_rows:,   obj_per_row, IMAGE_SIZE, sample_nchars):
    def __init__(self,parser:argparse, chars:list, prng:random,label_ids:list,samplei:int,sample_chars:int ):
        """
        Args:
            parser: The options parser.
            prng: The random generator.
            chars: The characters added so far.
        ...
        y, _ = shift.astype(np.int)
        origr, origc = np.unravel_index(samplei, [parser.num_rows_in_image, parser.nchars_per_row])
        c = origc
        r = origr
        _ , imageh , imagew = parser.image_size
        if c == 0:
            start_x = prng.randint(0,11) * parser.grit_size
            start_y = int(y)
            end_x = int(start_x + self.scale * self.letter_size)
            end_y = int(y + self.scale * self.letter_size)
        else:
         prev_char_x = chars[-1].end_x
         prev_char_y = chars[-1].end_y
         charcter_shift = prng.randint(0,11)
         start_x = int(prev_char_x + charcter_shift * parser.grit_size)
         end_x = int(prev_char_x + charcter_shift * parser.grit_size + self.scale * self.letter_size)
         start_y =int(y)
         end_y = int(y + self.scale * self.letter_size)
        #
        '''
        stx = int(c * parser.letter_size*1.25 + x)
        stx = max(0, stx)
        ...
        midx = int(np.rint((self.start_x + self.end_x)/2))
        midy = int(np.rint((self.start_y +self.end_y)/2))
