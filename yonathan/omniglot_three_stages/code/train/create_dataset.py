...
def gen_sample(parser:argparse, sample_id:int, is_train:bool, aug_data:transforms, dataloader:DataSet, example:ExampleClass, augment_sample:bool)->Sample:
    ...
    sample = Sample(infos, image, label_existence, label_ordered,sample_id, example.query_part_id_for_stages_1_2,flag,label_task,keypoints,example.query_part_id_for_stage_3)
    ...
    for k, (ds_type, cur_nexamples) in enumerate(zip(ds_types, nexamples_vec)):
        ...
        for i in range(cur_nexamples): # Iterating over the number of samples.
            ...
            for samplei in range(num_chars_per_image): #For each chosen character, we augment it and transform it.
                char = CharacterTransforms(parser,chars, prng, label_ids, samplei, sample_chars)
                chars.append(char)
            print(i)
