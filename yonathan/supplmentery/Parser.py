...
def GetParser(model_flag, processed_data, embedding_idx, results_dir, checkpoints_per_epoch=1):
    ...
    # TODO-understand what is gpu.
    parser.add_argument('--gpu', default=None, type=any,
                        help='Not clear - query!')
    ...
    parser.add_argument('--cycle_lr', default=True,
                        type=bool, help='Whether to cyclr the lr')
    parser.add_argument('--normalize_image', default=True, type=bool,
                        help='Whether to normalize the data by subtracting  the mean')
    parser.add_argument('--orig_relus', default=False, type=bool,
    ...
    parser.add_argument('--model_flag', default=model_flag,
                        type=FlagAt, help='Flag that defines the model type')
    parser.add_argument('--ntasks', default=4, type=int,
                        help='Number of tasks the model should handle')
    parser.add_argument('--nargs', default=6, type=int,
                        help='Number of possible position arguments in the image')
    parser.add_argument('--activation_fun', default=nn.ReLU,
                        type=nn.Module, help='Non Linear activation function')
    parser.add_argument('--use_bu1_loss', default=True, type=bool,
    ...
    parser.add_argument(
        '--ks', default=[7, 3, 3, 3], type=list, help='The kernel sizes')
    parser.add_argument(
    ...
    parser.add_argument('--norm_fun', default=BatchNorm,
    ...
    parser.add_argument('--num_gpus', default=num_gpus,
                        type=int, help='number of used gpus')
    ...
    parser.add_argument('--nclasses', default=[[47], [47], [47], [47]], type=list,
                        help='The sizes of the Linear layers')
    #  if Parser.flag_at is FlagAt.BU1_SIMPLE:
    #           Parser.ns = 3 * np.array(parser.ns)
    parser.add_argument('--logfname', default='log.txt',
                        type=str, help='The name of the log file')
    parser.add_argument('--bu2_loss', default=multi_label_loss,
                        type=Callable, help='The bu2 classification loss')
    parser.add_argument('--task_accuracy', default=multi_label_accuracy_base,
                        type=Callable,  help='The accuracy function')
    parser.add_argument('--ubs', default=1, type=int,
    ...
    if checkpoints_per_epoch > 1:
        learning_rates_mult = np.repeat(
            learning_rates_mult, heckpoints_per_epoch)
    setup_flag(parser)
    ...
    parser.add_argument('--save_model', default=True, type=bool,
                        help='Whether to save the model on the disk')
    ...
    parser.add_argument('--bu1_loss', default=nn.BCEWithLogitsLoss(reduction='mean').to(
        dev), type=nn.Module, help='The loss used at the end of the bu1 stream')
    ...
    parser.add_argument('--dataset_id', default='val', type=str,
