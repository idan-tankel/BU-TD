...
def main(train_right, train_left,ds_type=DsType.Emnist):
    parser = GetParser(task_idx = 0, direction_idx = 'right', flag=Flag.CL, ds_type = ds_type, model_type = BUTDModelShared)
    ...
    Checkpoint_saver = CheckpointSaver( dirpath='/home/sverkip/data/BU-TD/yonathan/Recognicion/data/emnist/results/MyFirstCkpt'+time, decreasing=False, top_n=5)
    ...
    training_flag = Training_flag(parser, train_all_model = False, train_arg = False, train_task_embedding = True, train_head = False)
    ...
    if train_left:
        [ train_dl, test_dl, val_dl, _, _, _] = get_dataset_for_spatial_realtions(parser, data_path,  lang_idx=0,    direction = 1)
