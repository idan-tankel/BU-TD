...
def define_wrapped_model(parser, training_flag, model, data_path, direction, lang_idx, Checkpoint_saver):
    ...
    Initial_DataLoaders = get_dataset_for_spatial_relations(parser, data_path, lang_idx=lang_idx,
                                                            direction_tuple=direction)
    wrapped_model = ModelWrapped(parser, model, learned_params, check_point=Checkpoint_saver,
                                 direction_tuple=direction,
                                 task_id=lang_idx,
                                 nbatches_train=len(Initial_DataLoaders['train_dl']), train_ds=Initial_DataLoaders[
            'train_ds'])
...
def main(flag=Flag.CL):
    ...
    if ds_type == "Emnist":
        ...
        Non_initial_tasks: list[tuple[tuple[int, int], int]] = [((0, 1), 0), ((1, 1), 0), ((-1, -1), 0),
                                                                ((2, 0), 0)]  # Four additional tasks.
    ...
    wrapped_model, Initial_DataLoaders = define_wrapped_model(parser, training_flag, model, data_path,
                                                              initial_direction, initial_task, Checkpoint_saver)
    trainer.fit(wrapped_model, train_dataloaders=Initial_DataLoaders['train_dl'],
                val_dataloaders=Initial_DataLoaders['test_dl'])
    ...
    if Data_type is DsType.Emnist or Data_type is DsType.FashionMnist:
        for direction, task in Non_initial_tasks:
            training_flag = Training_flag(parser, train_task_embedding=True,
                                          train_head=True)  # Train the embedding and the read-out head.
            wrapped_model, DataLoaders = define_wrapped_model(parser, training_flag, model, data_path,
                                                              direction, task, Checkpoint_saver)
            trainer.fit(wrapped_model, train_dataloaders=DataLoaders['train_dl'],
                        val_dataloaders=DataLoaders['test_dl'])
