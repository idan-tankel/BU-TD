...
def main(train_right, train_left, ds_type=DsType.Emnist, flag=Flag.CL):
    parser = GetParser(task_idx=0, direction_idx='right',
                       flag=flag, ds_type=ds_type, model_type=BUTDModelShared)
    project_path = Path(__file__).parents[1]
    data_path = os.path.join(
        project_path, 'data/{}/samples/18_extended'.format(ds_type.Enum_to_name()))
    ...
    Checkpoint_saver = CheckpointSaver(
        dirpath='/home/sverkip/data/BU-TD/yonathan/Recognicion/data/emnist/results/MyFirstCkpt'+time, decreasing=False, top_n=5)  # including reseting
    ...
    trainer = pl.Trainer(accelerator='gpu', max_epochs=60, logger=wandb_logger, callbacks=[
                         ModelCkpt], reload_dataloaders_every_n_epochs=True)
    training_flag = Training_flag(
        parser, train_all_model=True, train_arg=False, train_task_embedding=True, train_head=False)
    learned_params = training_flag.Get_learned_params(
        parser.model, lang_idx=0, direction=0)

    if train_right:
        [train_dl, test_dl, _] = get_dataset_for_spatial_realtions(
            parser, data_path, lang_idx=0, direction=0)
        model = ModelWrapped(parser, learned_params, ckpt=Checkpoint_saver,
                             direction_id=0, nbatches_train=len(train_dl))
