...
def get_dataset_for_spatial_realtions(args,data_fname, lan_idx: int,direction_idx:int) -> list:
    """
   ...
   # new_data_format = True
    ...
    train_ds = dataset(os.path.join(data_fname, 'train'), args.nclasses, args.ntasks,args.ndirections, lan_idx,direction_idx, args.nargs, nexamples=nsamples_train, split=True)
    ...
    train_ds = dataset(os.path.join(data_fname, 'train'), args.nclasses, args.ntasks,args.ndirections, lan_idx,direction_idx, args.nargs,
                       nexamples=nsamples_train, split=True, mean_image=mean_image)
    test_ds = dataset(os.path.join(data_fname, 'test'), args.nclasses, args.ntasks,args.ndirections, lan_idx,direction_idx, args.nargs,
                      nexamples=nsamples_test, split=True, mean_image=mean_image)
    if args.generelize:
     val_ds = dataset(os.path.join(data_fname, 'val'), args.nclasses, args.ntasks, lan_idx, direction_idx, args.nargs, nexamples=nsamples_test, split=True, mean_image=mean_image)
    else:
     val_ds = None
    ...
    # TODO - CCHANGE IT TO ONE IF-ELSE
    if args.generelize:
