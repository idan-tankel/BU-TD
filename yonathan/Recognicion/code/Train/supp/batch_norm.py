...
class BatchNorm(nn.Module):
    def __init__(self,opts, num_channels, dims=2):
        ...
        self.num_languages = opts.ntasks
        ...
        for _ in range(self.ndirections * self.num_languages):
    ...
    def load_running_stats(self, lan_id: int, direction_id:int) -> None:
        """

        :param task_emb_id:
        :return: loads the saved mean, variance to the running_mean,var in the test time.
        """
        task_emb_id = direction_id + self.ndirections * lan_id
    ...
    def store_running_stats(self, lan_id: int, direction_id:int) -> None:
        """
        :param task_emb_id:
...
def store_running_stats(model: nn.Module, lan_id: int, direction_id:int ) -> None:
    """
    ...
    for _, layer in model.named_modules():
        if isinstance(layer, BatchNorm):
            layer.store_running_stats(lan_id, direction_id)


def load_running_stats(model: nn.Module, lan_id: int, direction_id:int) -> None:
    """
    ...
    for _, layer in model.named_modules():
        if isinstance(layer, BatchNorm):
            layer.load_running_stats(lan_id, direction_id)
