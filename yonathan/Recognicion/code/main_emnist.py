...
def main(train_right, train_left, ds_type: DsType = DsType.Emnist, flag: Flag = Flag.CL, model_type: Module = BUTDModel,
    ...
    Checkpoint_saver = CheckpointSaver(
        dirpath=parser.results_dir +
                f'Model_use_reset_{str(task[0])}_wd_{str(parser.wd)}_base_lr_'
                f'{parser.base_lr}_max_lr_{parser.max_lr}_'
                f'epoch_ {epoch}_option_bs_{parser.bs}')
    ...
    if train_left:
        ...
        '''
        # 44
        wrapped_model.load_model(model_path=f'Model_(1, 0)_single_base/BUTDModel_epoch{epoch}_direction=[(1, 0)].pt')
        #   print(check['epoch'])
