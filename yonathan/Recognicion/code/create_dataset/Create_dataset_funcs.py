...
def store_sample_disk(parser: argparse, sample: Sample, store_dir: str) -> None:
    ...
    if parser.folder_split:
        ...
        samples_dir = os.path.join(store_dir, '%d' % (i // parser.folder_size))
    ...
    del sample.image
    # Dumping the labels, and the flags in the pickle file.
    ...
    for i in range(cur_nexamples):  # Iterating over the number of samples.
        ...
        if parser.generalize:  # If generalize we get the possible pairs we can choose from.
            sample_chars = Get_sample_chars(
                parser, prng, valid_pairs, ds_type, valid_classes, test_chars_list)
        else:
        ...
        create_examples_per_sample(
            parser, prng, ds_type, examples, sample_chars, chars, adj_types)
    return examples
...
def gen_sample(parser: argparse.ArgumentParser, sample_id: int, ds_type: str, dataloader: DataSet, example: ExampleClass) -> Sample:
    ...
    for char in example.chars:  # Iterate over each chosen character.
        ...
        image = AddCharacterToExistingImage(dataloader, image, char)
    ...
    sample = Sample(image, sample_id, label_existence, label_ordered,
                    example.query_part_id, label_task, flag, is_train)
...
def gen_samples(parser: argparse, dataloader: DataSet, job_id: int, range_start: int, range_stop: int, examples: list, storage_dir: str, ds_type: str) -> None:
    ...
    for k in range(len(ranges) - 1):  # Splitting into consecutive jobs.
        ...
        for samid in range(range_start, range_stop):
            ...
            sample = gen_sample(parser, samid, ds_type,
                                dataloader, examples[rel_id])
            if sample is None:
                continue
            # Stores the samples.
...
def Split_examples_into_jobs_and_generate_samples(parser: argparse.ArgumentParser, raw_data_set: DataSet, examples: list, storage_dir: str, ds_type: str):
    ...
    cur_njobs = min(njobs, np.ceil(cur_nexamples / job_chunk_size).astype(int))
...
def AddCharacterToExistingImage(DataLoader: DataSet, image: np.array, char: CharInfo) -> tuple:
    ...
    img_id = DataLoader.num_examples_per_character * label + label_id
...
def Get_label_task(example: ExampleClass, label_ordered: np.array, data_loader: data.Dataset) -> tuple:
    ...
    # The character information in the index.
    char = example.chars[query_part_id]
    ...
    edge_class = data_loader. nclasses
    # Find the row and the index of the character.
    r, c = (label_ordered == character).nonzero()
    r = r[0]
    c = c[0]
    ...
    for adj_type in adj_types:
        ...
        if single_feat_to_generate or not is_train:
            ...
            query_part_ids = prng.choice(
                valid_queries, ngenerate, replace=False)
...
# Only for emnist.
def Get_valid_classes_for_emnist_only(ds_name, use_only_valid_classes: bool, nclasses: int) -> np.array:
    """
...
def Make_data_dir(parser: argparse.ArgumentParser, ds_type, language_list: list) -> tuple:
    ...
    """
    try:
        num_characters_per_image = parser.num_characters_per_image
    except AttributeError:
        num_characters_per_image = parser.nchars_per_row * parser.num_rows_in_the_image
    base_storage_dir = f'{num_characters_per_image}_'
...
def create_samples(parser: argparse, ds_type, raw_data_set: DataSet, language_list) -> dict:
    ...
    # The number of validation samples we desire to create.             # The number of queries to create for each sample.
    ...
    if generalize:  # if we want also the CG dataset we add its name to the ds_type and the number of needed samples.
        ...
        ds_types.append('val')
        valid_pairs, CG_chars_list = Get_valid_pairs_for_the_combinatorial_test(
            parser, nclasses, valid_classes)  # Generating valid pairs sampling from.
    else:
    ...
    for k, (ds_type, cur_nexamples) in enumerate(zip(ds_types, nexamples_vec)):
        ...
        Split_examples_into_jobs_and_generate_samples(
            parser, raw_data_set, examples, storage_dir, ds_type)
...
def Save_meta_data_and_code_script(parser: argparse, ds_type, nsamples_per_data_type_dict: dict, language_list: list):
    ...
    """
    storage_dir, meta_data_fname = Make_data_dir(
        parser, ds_type, language_list)
...
def Save_script_if_needed(storage_dir: str):
    ...
    if not os.path.exists(storage_dir):
        shutil.copytree(code_folder_path, storage_dir,
