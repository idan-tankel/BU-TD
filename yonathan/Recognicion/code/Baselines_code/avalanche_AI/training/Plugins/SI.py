...
class SynapticIntelligencePlugin(SupervisedPlugin):
    ...
    def __init__(  self,parser, eps: float = 0.0000001, excluded_parameters: Sequence["str"] = None, device: Any = "as_strategy", ):
        ...
        if excluded_parameters is None:
            excluded_parameters = []
        self.si_lambda = (
            parser.si_lambda if isinstance(parser.si_lambda, (list, tuple)) else [parser.si_lambda]
        )
        self.eps: float = eps
    ...
    def update_ewc_data(
        ...
        for param_name in ewc_data[1]:
            ewc_data[1][param_name] = torch.clamp(
                ewc_data[1][param_name],min = 0.0, max=clip_to
            )
...
class MySynapticIntelligencePlugin(SynapticIntelligencePlugin):
    ...
    def Initiliaze_pretrained_model_importances(self, strategy):
        """
        Initializing the importances of a pretrained model.
        Args:
    ...
    def before_training_exp(self, strategy: "SupervisedTemplate", **kwargs):
        SupervisedPlugin.before_training_exp(self, strategy, **kwargs)
        if strategy.EpochClock.just_initialized:
    ...
    def before_training_iteration(self, strategy: "SupervisedTemplate", **kwargs ):
        ...
        MySynapticIntelligencePlugin.pre_update(strategy.model, self.syn_data, self.excluded_parameters)

    def after_training_iteration(self, strategy: "SupervisedTemplate", **kwargs):
        SupervisedPlugin.after_training_iteration(self, strategy, **kwargs)
        MySynapticIntelligencePlugin.post_update(strategy.model, self.syn_data, self.excluded_parameters)
    ...
    def update_ewc_data(net, ewc_data: EwcDataType, syn_data: SynDataType, clip_to: float, excluded_parameters: Set[str], c=0.0015, eps: float = 0.0000001, ):
        MySynapticIntelligencePlugin.extract_weights( net, syn_data["new_theta"], excluded_parameters)

        for param_name in syn_data["cum_trajectory"]:
            syn_data["cum_trajectory"][param_name] += ( c  * syn_data["trajectory"][param_name] / ( np.square(syn_data["new_theta"][param_name] - ewc_data[0][param_name]    ) + eps ) )
    ...
    @staticmethod
    @torch.no_grad()
    def init_batch( model,  ewc_data: EwcDataType, syn_data: SynDataType,  excluded_parameters: Set[str]):
        # Keep initial weights
    ...
    def post_update(  model, syn_data: SynDataType, excluded_parameters: Set[str] ):
        MySynapticIntelligencePlugin.extract_weights( model, syn_data["new_theta"], excluded_parameters)
        MySynapticIntelligencePlugin.extract_grad(   model, syn_data["grad"], excluded_parameters )
        for param_name in syn_data["trajectory"]:
            syn_data["trajectory"][param_name] += syn_data["grad"][param_name] * (syn_data["new_theta"][param_name]- syn_data["old_theta"][param_name] )
