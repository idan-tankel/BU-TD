...
def main(train_right, train_left, ds_type=DsType.Emnist, flag=Flag.CL, model_type = BUTDModel, task = (-1, 0)):
    ...
    results_dir = os.path.join(project_path, 'data/{}/results/model'.format(ds_type.Enum_to_name()))
    data_path = os.path.join(project_path, 'data/{}/samples/9_extended_testing'.format(ds_type.Enum_to_name()))
    ...
    Checkpoint_saver = CheckpointSaver(dirpath=results_dir + time)
