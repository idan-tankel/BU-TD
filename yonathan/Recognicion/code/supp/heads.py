...
class HeadSingleTask(nn.Module):
    ...
    def __init__(self, opts: argparse, nclasses: list) -> None:
        ...
        layers = []
        for k in range(
                opts.nheads):  # according to  the output size we allocate the number of heads.if flag=NOFLAG all characters(usually 6) will be recognized,the loop will run 6 times.
