...
class ChannelModulation(nn.Module):
    ...
    def __init__(self, nchannels: int) -> None:
        ...
        self.weights = nn.Parameter(torch.Tensor(*shape))  # creates the learnable parameter of shape [nchannels,1,1] according to nchannels.
...
class SideAndComb(nn.Module):
    ...
    def __init__(self,parser, filters: int) -> None:
        """
        Args:
        ...
        self.norm = parser.norm_layer(filters,parser.ntasks)  # batch norm after the channel-modulation of the lateral.
        self.orig_relus = parser.orig_relus
        self.filters = filters
        if not parser.orig_relus:
            self.relu1 = parser.activation_fun()  # activation_fun after the batch_norm layer
        self.relu2 = parser.activation_fun()  # activation_fun after the skip connection
...
class Modulation(nn.Module):  # Modulation layer.
    def __init__(self,opts, inshapes: list, pixel_modulation: bool, ntasks: int) -> None:
        """
        Args:
            inshapes: The shape to create the modulation according to.
       ...
       # bias = False
        for i in range(ndirections):  # allocating for every task its task embedding
            layer = nn.Linear(1, inshapes, bias = False)
    ...
    def forward(self, inputs: torch, flag: torch) -> torch:
        """
        Args:
            inputs: Inout to modulate, the output from the Conv block.
            flag: The task flag.

        Returns:A modulated tensor of the same shape.
        """
...
class BUInitialBlock(nn.Module):
    def __init__(self, opts: argparse, shared: nn.Module) -> None:
        ...
        self.conv1 = nn.Sequential(shared.conv1, opts.norm_layer(self.filters, opts.ntasks), self.activation_fun())  # The initial block downsampling from RGB.
...
class BasicBlockBU(nn.Module):
    ...
    def __init__(self,opts, shared: nn.Module, inshapes: torch, is_bu2: bool) :
        """
        Args:
            opts: The model options.
        ...
        if self.flag_at is Flag.SF and self.is_bu2:  # If BU2 stream create the task embedding.
            self.task_embedding = [[] for _ in range(opts.ntasks)]  # The parameters stored as task embedding.
            self.task_embedding_layers = []
            self.channel_modulation_after_conv1 = Modulation(opts,nchannels, False,
                                                             self.ndirections)  # channel modulation after conv1
            self.task_embedding_layers.append(self.channel_modulation_after_conv1)
            self.pixel_modulation_after_conv1 = Modulation(opts,shape_spatial, True,
                                                           self.ndirections)  # pixel modulation after conv1
            self.task_embedding_layers.append(self.pixel_modulation_after_conv1)
            self.channel_modulation_after_conv2 = Modulation(opts, nchannels, False,
                                                             self.ndirections)  # channel modulation after conv2
            self.task_embedding_layers.append(self.channel_modulation_after_conv2)
            self.pixel_modulation_after_conv2 = Modulation(opts, shape_spatial, True,
                                                           self.ndirections)  # pixel modulation after conv2
        ...
        self.conv1 = nn.Sequential(shared.conv1, opts.norm_layer(nchannels, self.ntasks), opts.activation_fun())  # conv1
        self.conv2 = nn.Sequential(shared.conv2, opts.norm_layer(nchannels, self.ntasks), opts.activation_fun())  # conv2
        if shared.downsample is not None:
            downsample = nn.Sequential(shared.downsample, opts.norm_layer(nchannels,
...
class InitialTaskEmbedding(nn.Module):
    ...
    def __init__(self, opts: argparse):
        ...
        if self.model_flag is Flag.SF:
            ...
            for i in range(self.ndirections):
                layer = nn.Sequential(nn.Linear(1, self.top_filters // 2), self.norm_layer(self.top_filters // 2, dims=1, num_tasks=self.ndirections),  self.activation_fun())
    ...
    def forward(self, inputs: tuple) -> tuple:
        ...
        if self.ds_type is DsType.Omniglot:
           ...
           task_id = flag_to_task(direction_flag)
           arg_id = flag_to_task(lan_flag)
           flag_task = direction_flag[:, task_id].view(-1, 1).clone()
        else:
            task = flag[:, :self.ntasks]  # The task vector.
            arg = flag[:, self.ntasks:]  # The argument vector.
            task_id = flag_to_task(task)
            flag_task = flag[:, task_id].view(-1, 1)
        if self.model_flag is Flag.SF:
            top_td_task = self.h_flag_task_td[task_id](flag_task)  # Take the specific task embedding to avoid forgetting.
        else:
            top_td_task = self.h_flag_task_td(task)
        top_td_task = top_td_task.view((-1, self.top_filters // 2, 1, 1))
        if self.train_arg_emb:
         top_td_arg = self.h_flag_arg_td[arg_id](arg)  # Embed the argument.
        else:
...
class BasicBlockTD(nn.Module):
    ...
    def __init__(self,opts, in_channels, out_channels, stride, inshapes, index):
        """
        Args:
            opts: The model options.
        ...
        self.conv1_norm = opts.norm_layer(in_channels, self.ntasks)
        self.relu1 = opts.activation_fun()
        self.conv2 = conv3x3up(in_channels, out_channels,size, stride)
        self.conv2_norm = opts.norm_layer(out_channels, self.ntasks)
        if self.orig_relus:
        ...
        if stride != 1:
            upsample = nn.Sequential(nn.Upsample(size = size, mode='bilinear', align_corners=False),
                                     conv1x1(in_channels, out_channels, stride=1), opts.norm_layer(out_channels, opts.ntasks))
        elif in_channels != out_channels:
            upsample = nn.Sequential(conv1x1(in_channels, out_channels, stride=1), opts.norm_layer(out_channels, opts.ntasks))
