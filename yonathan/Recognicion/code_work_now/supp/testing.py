...
def Create_and_Initialize_Modulation( outchannels, bias):
    if bias:
     layer = nn.Linear(1, inshapes)
    else:
      layer = nn.Linear(1, outchannels, bias = False)
      weight1 = torch.zeros(outchannels, 1)
      init.uniform_(weight1,-1,1)
      weight2 = torch.zeros(outchannels, 1)
      init.uniform_(weight2, -1, 1)
      layer.weight = nn.parameter.Parameter(weight1 + weight2)
    return layer
...
class BasicBlockBU(nn.Module):
    ...
    def __init__(self, shared: nn.Module, norm_layer: nn.Module, activation_fun: nn.Module, inshapes: torch,
                 ntasks: int, flag_at: Flag, is_bu2: bool, orig_relus) -> None:
        """
