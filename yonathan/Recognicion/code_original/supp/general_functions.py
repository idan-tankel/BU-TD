...
def get_laterals(laterals: list[torch], layer_id: int, block_id: int) -> torch:
    """
    Returns the laetral connections associated with the layer, blpck.
    Args:
...
class depthwise_separable_conv(nn.Module):
    ...
    def __init__(self, channels_in: int, channels_out: int, kernel_size: int, stride: int = 1, padding: int = 1, bias: bool = False) -> None:
        """
        Args:
...
def conv3x3up(in_channels, out_channels,size, upsample = False) -> nn.Module:
    """
    Args:
