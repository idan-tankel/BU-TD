...
class SideAndCombSharedBase(nn.Module):
    # Class saving the needed number of filters in the lateral connection.
...
class Modulation(nn.Module):  # Modulation layer.
    def __init__(self,opts, shape: list, pixel_modulation: bool):
        ...
        self.task_embedding = [[] for _ in range(opts.ndirections)]
        self.modulation = []
...
class BasicBlockBUShared(nn.Module):
    ...
    expansion = 1
    def __init__(self,opts, in_channels: int, out_channels: int, stride: int,idx:int=0):
        """
        Args:
            opts: The model options.
...
class BasicBlockBU(nn.Module):
    ...
    def __init__(self,opts, shared: nn.Module,  inshapes: torch, is_bu2: bool) -> None:
        """
        Args:
            opts: The model options.
            shared: The shared part between BU1, BU2.
