...
def classification_accuracy(outs, labels):
    preds = outs.argmax(dim = 1).squeeze()
...
CE_LOSS = nn.CrossEntropyLoss(reduction='none').to(dev)
...
class UnifiedLossFun:
    ...
    def __call__(self, model, inputs: list[torch], outs: list) -> float:
        ...
        """
        outs = outs[0].squeeze() # The output from all the streams.

        loss_task = CE_LOSS(outs,inputs[1])
        loss = loss_task.mean()
        return loss
