...
def train_model(args: argparse, the_datasets: list, learned_params: list, task_id: int) -> None:
    ...
    if args.SGD:
        optimizer = optim.SGD(learned_params, lr=args.initial_lr, momentum=args.momentum, weight_decay=args.wd)
        lambda_lr = lambda epoch: args.lr_decay
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, lambda_lr,  last_epoch=-1)
    else:
        lambda_lr = lambda epoch: args.lr_decay**epoch
        optimizer = optim.Adam(learned_params, lr=args.initial_lr, weight_decay=args.wd)
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, lambda_lr, last_epoch=-1)

    args.optimizer = optimizer
    args.scheduler = scheduler
    return fit(args, the_datasets, task_id)
...
def save_model_and_md(logger: logging, model_fname: str, metadata: dict, epoch: int, opts: argparse) -> None:
    ...
    torch.save({'epoch': epoch, 'model_state_dict': opts.model.state_dict(),
                ...
                'scheduler_state_dict': opts.scheduler.state_dict() if opts.scheduler != None else None, 'metadata': metadata, },
