...
class BUInitialBlock(nn.Module):
    def __init__(self, opts: argparse, shared: nn.Module) -> None:
        ...
        self.conv1 = nn.Sequential(shared.conv1, opts.norm_fun(self.filters, opts.ntasks), self.activation_fun())  # The initial block downsampling from RGB.
        self.bot_lat = SideAndCombShared(shared.bot_lat, self.norm_layer, self.activation_fun, self.orig_relus, self.ntasks)  # Skip connection from the TD initial embedding.
