...
def main():
    ...
    if storage_disk:
        ...
        if cmd_args.extended:
            base_storage_dir += 'extended50k'
        else:
            base_storage_dir += 'sufficient'

        fold_n = 1
        base_storage_dir =  base_storage_dir + str(fold_n)
        # base_storage_dir = 'tmpemnist_adj_2dir_6_ids_10k_aug_genmore1nb_5cs'
        # base_storage_dir = 'tmpemnist'
        ...
        os.makedirs(base_samples_dir, exist_ok=False)
    ...
    emnist_preprocess.convertor = 'tf' #'pytorch'
    if emnist_preprocess.convertor == 'pytorch':
    ...
    if generalize:
        ...
        # For 24 characters, 1 string excludes about 2.4% pairs of consequtive characters, 4 strings: 9.3%, 23 strings: 42%, 37: 60%
...
if __name__ == "__main__":
    ...
    if ret:
        memsamples = ret
# %%
# %% validate/visualize the generated set
if memsamples:
    emnist_preprocess = SimpleNamespace()
    emnist_preprocess.mapping_fname = os.path.join(
        'data/emnist', "emnist-balanced-mapping.txt")
    cl2let = get_cl2let(emnist_preprocess)
    it = 1
    import v26.cfg as cfg
    cfg.gpu_interactive_queue = True
    from v26.funcs import *
    for k in range(100):
        sample = memsamples[it][k]
        plt.imshow(sample.image)
        print(cl2let[sample.flag[1]], sample.flag[0],
              cl2let[sample.label_task])
        pause_image()
# %% what is the distribution of the labels
#samples=memsamples[0]
#adj_labels = np.array(
#        [sample.label_task for sample in samples])
#a=np.bincount(adj_labels)
