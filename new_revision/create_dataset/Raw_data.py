...
def Get_raw_data(download_dir: str, dataset: DsType, language_list, raw_data_source) -> tuple:
    ...
    if dataset.from_enum_to_str() == 'omniglot':
        ...
        for lan_idx in language_list:
            ...
            language_images = [os.path.join(
                dp, f) for dp, dn, filenames in os.walk(lan) for f in filenames]
            language_images.sort()
            language_images = [transform(skimage.io.imread(
                im_file)) for im_file in language_images]
            images_arranged.extend(language_images)
        num_images_per_label = 20
        num_labels = len(images_arranged) // 20
        labels_arranged = sum([num_images_per_label * [i]
        ...
        if dataset.from_enum_to_str() == 'emnist':
            ...
            train_raw_dataset = torchvision.datasets.FashionMNIST(
                download_dir, train=True, download=True)
            test_raw_dataset = torchvision.datasets.FashionMNIST(
                download_dir, train=False, download=True)
            shape = (1, 28, 28)

        elif dataset.from_enum_to_str() == 'kmnist':
            train_raw_dataset = torchvision.datasets.KMNIST(
                download_dir, train=True, download=True)
            test_raw_dataset = torchvision.datasets.KMNIST(
                download_dir, train=False, download=True)

        images = [np.array(train_raw_dataset[i][0]).reshape(shape)
                  for i in range(len(train_raw_dataset))]
        images.extend([np.array(test_raw_dataset[i][0]).reshape(shape)
                      for i in range(len(test_raw_dataset))])
        labels = [train_raw_dataset[i][1]
                  for i in range(len(train_raw_dataset))]
        labels.extend([test_raw_dataset[i][1]
                      for i in range(len(test_raw_dataset))])
        letter_size = images[0].shape[1]
        num_labels = len(set(labels))
        num_images_per_label = len(images) // num_labels
        # Arranging the data according to the labels.
        images_arranged = sum([[images[i] for i in range(
            len(images)) if labels[i] == k] for k in range(num_labels)], [])
        labels_arranged = sum(
            [[k] * num_images_per_label for k in range(num_labels)], [])
...
class DataSet(data.Dataset):
    def __init__(self, parser, data_dir: str, dataset: DsType, language_list: list, raw_data_source: str):
        ...
        # The path we download the raw data into.
        download_raw_data_dir = os.path.join(
            data_dir, f'{dataset.from_enum_to_str()}_raw', )
        self.images, self.labels, self.letter_size, self.nchannels = Get_raw_data(
            download_raw_data_dir, dataset=dataset, language_list=language_list, raw_data_source=raw_data_source)
