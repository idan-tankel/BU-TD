...
def train_emnist(embedding_idx=1, flag_at=FlagAt.SF,
    ...
    train_dl, test_dl, val_dl, train_dataset,test_dataset = get_dataset(direction=embedding_idx, args=config, data_fname=data_path)
    the_datasets = [train_dl, test_dl, val_dl]
    # Printing the model and the hyper-parameters.
    logger.print_detail(config)
    # creating the model according the parser.
    model = create_model(model_opts=config)
    measurments.set_datasets_measurements(datasets=the_datasets, measurements_class=measurments.Measurements, model_opts=config, model=model)
    ...
    if path_loading is not None or config.Training.load_existing_path:
        ...
        training_functions.load_model(config, results_dir, path_loading,model=model)
    ...
    training_functions.train_model(
        config, the_datasets, learned_params, embedding_idx, model)
