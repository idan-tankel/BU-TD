...
from ..Module_Blocks import conv3x3up, \
    Modulation_and_Lat, layer_with_modulation_and_masking
from ..Batch_norm import BatchNormForSpatialRelations as BatchNorm
...
class BasicBlockBUShared(nn.Module):
    ...
    def __init__(self, opts: argparse, in_channels: int, out_channels: int, stride: int):
        ...
        self.use_lateral = opts.data_obj['use_lateral_tdbu']  # Whether to create the BU -> TD laterals.
        self.conv1 = conv3x3(in_planes=in_channels, out_planes=out_channels,
                             stride=stride)  # changes the
        # number of channels and the
        # spatial shape.
        self.conv2 = conv3x3(in_planes=out_channels, out_planes=out_channels)  # preserves the tensor shape.
        ...
        if stride != 1 or in_channels != out_channels * BasicBlockBUShared.expansion:
            ...
            self.downsample = conv1x1(in_planes=in_channels, out_planes=out_channels * BasicBlockBUShared.expansion,
                                      stride=stride)
        if self.use_lateral:
...
class BUInitialBlock(nn.Module):
    ...
    def __init__(self, opts: argparse, shared: nn.Module, is_bu2: bool = False):
        ...
        self.bn1 = BatchNorm(opts, opts.data_obj['nfilters'][0])
        self.relu = nn.ReLU(inplace=True)
        self.use_laterals = opts.data_obj['use_lateral_tdbu']
        if self.use_laterals and is_bu2:
...
class BasicBlockBU(nn.Module):
    ...
    def __init__(self, opts: argparse, shared: nn.Module, block_inshapes: Tensor, is_bu2: bool,
        ...
        self.conv1 = layer_with_modulation_and_masking(opts=opts, layer=shared.conv1,
                                                       task_embedding=task_embedding, create_modulation=True,
                                                       create_masks=True, masks=masks, linear=False)
        self.bn1 = BatchNorm(opts, nchannels)
        self.conv2 = layer_with_modulation_and_masking(opts=opts, layer=shared.conv2,
                                                       task_embedding=task_embedding, create_modulation=True,
                                                       create_masks=True, masks=masks, linear=False)
        ...
        if shared.downsample is not None:
            ...
            self.conv1x1 = layer_with_modulation_and_masking(opts=opts, layer=shared.downsample,
                                                             task_embedding=task_embedding, create_modulation=True,
                                                             create_masks=True, masks=masks, linear=False)
...
class InitialEmbeddingBlock(nn.Module):
    ...
    def __init__(self, opts: argparse):
        ...
        self.top_filters = opts.data_obj['nfilters'][-1]
        ...
        if self.model_flag is Flag.CL or self.model_flag is Flag.Read_argument:
            ...
            self.top_td_arg_emb = nn.ModuleList(
                [nn.Linear(in_features=self.nclasses, out_features=self.top_filters) for _ in range(self.ntasks)])
    ...
    def forward(self, bu_out: Tensor, samples: Spatial_Relations_inputs_to_struct) -> Tensor:
        """
        Args:
...
class BasicBlockTD(nn.Module):
    ...
    def __init__(self, opts: argparse, in_channels: int, out_channels: int, stride: int, block_inshape: np.ndarray,
                 index: int, task_embedding, masks):
        """
        Args:
            opts: The model options.
        ...
        self.index = index
        self.use_lateral = opts.data_obj['use_lateral_butd']  # Whether to use the laterals from TD to BU.
        self.relu = nn.ReLU(inplace=True)
        self.stride = stride
        ...
        self.conv1 = conv3x3(in_planes=in_channels, out_planes=in_channels)
        ...
        #
        self.conv2 = conv3x3(in_planes=in_channels, out_planes=out_channels)
        self.modulated_conv2 = layer_with_modulation_and_masking(opts=opts, layer=self.conv2,
                                                                 task_embedding=task_embedding, create_modulation=True,
                                                                 create_masks=True, masks=masks, linear=False)
        # Changing the number of
        # channels.
        if stride > 1:  # Adding upsample layer.
            self.conv_upsample = nn.Upsample(size=size, mode='bilinear', align_corners=False)
            # Upsample the inner dimensions of the tensor.
        #
        ...
        if stride > 1:
            ...
            self.conv1x1 = conv1x1(in_planes=in_channels, out_planes=out_channels)
            self.modulated_conv1x1 = layer_with_modulation_and_masking(opts=opts, layer=self.conv1x1,
                                                                       task_embedding=task_embedding,
                                                                       create_modulation=True,
                                                                       create_masks=True, masks=masks, linear=False)
    ...
    def forward(self, x: Tensor, samples: Tensor, laterals: Tensor) -> tuple[Tensor, list[Tensor]]:
        ...
        x = self.modulated_conv1(x, samples.task)  # Perform the first conv block.
        ...
        laterals_out.append(x)
        x = self.modulated_conv2(x, samples.task)  # Perform the first conv block.
        if self.stride > 1:
        ...
        if self.upsample is not None:
            ...
            identity = self.modulated_conv1x1(identity, samples.task)
