...
from persons.code.v26.models.AutoSimpleNamespace import AutoSimpleNamespace
...
class AvatarDetailsDataset(AvatarDetailsDatasetBase):
    def __getitem__(self, index):
        ...
        data_fname = os.path.join(root, '%d_raw.pkl' % index)
        with open(data_fname, "rb") as new_data_file:
            sample = pickle.load(new_data_file)
        # lbl, flag, lbl_task, id
        id = sample.id
        lbl = sample.label_all
        flag = sample.flag
        lbl_task = sample.label_task
...
class AvatarDetailsDatasetLabelAll(AvatarDetailsDataset):
    ...
    def __getitem__(self, index):
        basic_inputs = super().__getitem__(index)  # TODO - check code of super class - wrong import: 0_data.pkl
        sample = self.get_raw_sample(index)
        ...
        return (*basic_inputs, label_all, loss_weight, sample.person_features)
...
class AvatarDetailsDatasetRaw(AvatarDetailsDatasetBase):
    def __getitem__(self, index):
        ...
        loss_weight = loss_weight.float()
        return img, label_existence, flag, seg, label_task, id, label_all, loss_weight, person_features
